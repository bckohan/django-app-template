...
A Django application library. Source lives in `src/{{cookiecutter.package_name}}/`. Tests are in `tests/`. Documentation is in `doc/`.
...
```
src/{{cookiecutter.package_name}}/   # library source
tests/
